    ...
    try:
        with request.urlopen(url) as resp:
            ...
            if isinstance(data, list) and data and isinstance(data[0], dict) and data[0].get("one") in (1, "1"):
                print("SELECT (direct) test PASSED")
                return 0
            else:
                print("SELECT (direct) test FAILED: unexpected response payload")
                return 1
    except error.HTTPError as e:
        body = e.read().decode("utf-8", errors="replace")
        print(f"HTTPError {e.code}")
        print(body)
        print("SELECT (direct) test FAILED: endpoint returned error")
        return 1
    except Exception as e:
        print(f"SELECT (direct) test FAILED: {e}")
        return 1
